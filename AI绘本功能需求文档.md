...
| V1.1 | 2025-11-04 | 开发团队 | 新增第4章"用户认证与登录"，补充登录逻辑说明 |
...
-   **首页浏览**：用户可以无需登录，直接在首页浏览精选绘本。
-   **登录校验触发场景**：
    -   点击"我的绘本"时
    -   点击"生成大纲"进行创作时
-   **未登录时**：若用户未登录，系统自动跳转至登录页，登录成功后返回首页。
-   **主动登录**：用户可在首页顶部点击"登录"按钮，跳转至登录页进行登录。
-   **退出登录**：用户退出登录后，系统自动跳转至首页。
-   **输入内容保留**：如果用户在输入框内已填写内容，跳转登录时需保留该内容（通过 localStorage 或 sessionStorage），登录成功返回首页后自动恢复到输入框中。
...
为了提升用户输入体验，在故事灵感输入框右下角提供语音输入功能，用户可以通过语音快速输入故事创意。

-   **交互流程与状态**:
    -   **初始状态（待录音）**:
        -   **UI表现**: 显示麦克风图标 🎤，Tooltip 显示"语音输入"，背景为默认样式
        -   **触发条件**: 用户点击麦克风按钮
        -   **转换行为**:
            -   切换为录音中状态
            -   开始采集麦克风音频并实时识别
            -   记录录音开始时输入框的原有内容
    -   **录音中状态**:
        -   **UI表现**: 显示动态波点图标 `...`，Tooltip 显示"停止语音输入"，背景变为浅蓝色高亮
        -   **功能表现**: 识别到的文字实时更新显示在输入框中
        -   **停止条件**（满足任一条件即停止）:
            -   用户再次点击按钮
            -   用户直接点击输入框
            -   字数达到上限（2000字符）
...
-   **文本处理**:
    -   录音开始时，保存输入框中的原有内容
    -   识别过程中，流式返回的是**完整的识别结果**（非增量）
    -   每次收到识别结果时，将"原有内容 + 识别结果"更新到输入框
    -   实时检测字符长度，如果"原有内容 + 识别结果"超过最大字符限制：
        -   自动停止录音
        -   按钮恢复为待录音状态
        -   保留已识别的内容（截取到最大长度）
        -   显示 Toast 提示："已达到字数上限，语音输入已停止"
    -   录音结束后，最终识别文本追加到原有内容末尾
-   **技术实现(参考)**:
    -   **语音识别服务**: 使用讯飞实时语音转写 API
    -   **识别模式**: 实时流式识别，每次返回完整识别结果
-   **异常处理**:
    -   **麦克风权限被拒绝**: 显示 Toast 提示"请允许麦克风权限以使用语音输入"
    -   **网络连接失败**: 显示 Toast 提示"语音识别服务连接失败，请检查网络"
    -   **录音时长限制**: 超过 200 秒自动停止录音，显示 Toast 提示"录音时间过长，已自动停止"
...
3.  **过程**:
    ...
    -   后端以JSON流的形式逐步返回大纲数据，前端通过识别JSON流中的特定关键词来判断生成进度并更新状态。进度分为四个固定阶段，各阶段状态依次流转。流程如下：
...
-   **主要角色**:
    ...
    -   **校验时机与交互**:
        ...
        -   **约束**: 
            -   **最少角色**: 至少保留一个角色。若尝试删除最后一个角色，提示"至少需要保留一个主要角色！"。
            -   **最多角色**: 最多添加10个角色。达到上限后，"添加角色"按钮置灰（透明度50%，光标显示not-allowed）。若点击置灰按钮，提示"最多只能添加10个角色！"。删除角色后，若数量低于10个，按钮自动恢复可用状态。
-   **主要场景**:
    -   **UI组件**: 列表形式展示，每个场景包含：
    ...
    -   **校验时机与交互**:
        ...
        -   **校验时机**: 在点击"生成绘本"按钮时进行校验。若不符合规则，卡片方显示相应的错误提示。
        -   **约束**: 
            -   **最少场景**: 至少保留一个场景。若尝试删除最后一个场景，提示"至少需要保留一个主要场景！"。
            -   **最多场景**: 最多添加10个场景。达到上限后，"添加场景"按钮置灰（透明度50%，光标显示not-allowed）。若点击置灰按钮，提示"最多只能添加10个场景！"。删除场景后，若数量低于10个，按钮自动恢复可用状态。
...
-   **状态**: 等待中 → 进行中 → 已完成
-   **等待提示**: "等待编写详细的故事内容..."
-   **进行中提示** (动态变化，基于流式输出识别关键词)：
    -   识别到 `"character_models"` → "正在生成角色模型..."
    -   识别到 `"scene_models"` → "正在构建场景描述..."
    -   识别到 `"cover"` → "正在设计封面..."
    -   识别到 `"page_number": N` → "正在生成第 N 页..."
-   **完成提示**: "已编写完整的故事脚本"
-   **预计时长**: 8-12 秒（根据页数）
...
**1. 动态进度识别** (阶段 2)
-   **实现方式**: 通过 `updateScriptProgress(streamContent)` 方法实时分析流式输出
-   **关键词映射**:
    | 关键词 | 提示文本 |
    |-------|---------|
...
-   **优势**: 提供更精确的进度反馈，替代传统的固定等待时间

**2. 具体进度计数** (阶段 3)
-   **实现方式**: 通过 `updateImageProgress(current, total)` 方法更新
-   **显示格式**: "正在生成插图 3/20"
-   **优势**: 用户可以清楚知道剩余图片数量和当前进度

**3. 组件化设计**
-   **类名**: `GenerationProgress`
-   **文件位置**: `js/generation-progress.js`
-   **复用场景**: 
    -   首页 (`home.html`) - 立即生成功能
    -   我的绘本页 (`my-books.html`) - 继续生成/重新生成
-   **核心方法**:
    ```javascript
    // 显示进度窗口
    progress.show(bookId, currentStep);
    
    // 更新脚本生成进度（自动识别关键词）
    progress.updateScriptProgress(streamChunk);
...
**正常完成**:
-   所有步骤完成后，等待 2 秒展示"已完成"状态
-   "生成绘本进度"窗口自动关闭
-   **首页生成**: 自动打开绘本阅读器，展示生成的绘本
-   **我的绘本页生成**: 更新卡片状态为"已完成"，显示成功提示

#### g. 用户交互

**1. "在后台继续"按钮**
-   **功能**: 点击后立即关闭进度窗口，但生成任务在后台继续
-   **适用场景**: 用户想继续浏览网站其他内容
-   **实现**: 调用 `progress.close()` 方法，但不中断生成流程
-   **用户反馈**: 用户可以在"我的绘本"页面查看生成状态和最终结果

**2. "取消生成"按钮**
-   **功能**: 中止当前生成任务
-   **确认流程**:
    1. 点击按钮
    2. 弹出二次确认对话框："确定要取消生成吗？"
    3. 用户确认后，调用 `progress.cancel()` 方法
-   **后端处理**:
    -   立即停止所有正在进行的 AI 模型调用
    -   停止流式输出
    -   取消图片生成队列
...
4. **接口返回数据格式错误**: 
   -   后端返回的数据不是预期的 JSON 格式
   -   缺少必要字段（如 `pages`, `cover` 等）
   -   数据类型不匹配
...
2. **错误提示信息** (根据不同错误类型)：
   ...
   | 生成失败 | "绘本生成失败，请稍后再试或联系管理员。" |
   | 请求超时 | "请求超时，生成绘本时间过长，请稍后再试。" |
   | 数据格式错误 | "服务器返回数据异常，请联系技术支持。" |

3. **后端状态更新**:
   -   将对应绘本的状态更新为 `"generation_failed"`
   -   保存错误信息到数据库（包含错误类型、时间戳、详细信息）
   -   发送错误通知给管理员（如果是系统级错误）

4. **用户后续操作**:
   -   用户在"我的绘本"页面可以看到状态为"生成失败"
   -   提供"重新生成"按钮，用户可以再次尝试
   -   保留原有的故事大纲数据，无需重新填写

**错误恢复策略**:
-   **自动重试**: 对于网络临时中断，系统可尝试自动重连（最多 3 次）
-   **断点续传**: 如果阶段 2 完成但阶段 3 失败，可以只重新生成插图
-   **降级处理**: 如果图片生成失败，可以使用默认占位图，但脚本仍然保存
...
1.  **页面导航**:
    ...
    -   **区域点击翻页**: 
        -   **左侧图片区域**: 点击左侧图片容器的任意位置，可翻到上一页。
        -   **右侧文字区域**: 点击右侧文字容器的任意位置，可翻到下一页。
        -   **排除规则**: 点击编辑按钮（"编辑文字"、"编辑图片"）、页码显示、"重新开始"按钮时不触发翻页。
        -   **编辑模式禁用**: 在编辑模式下，区域点击翻页功能被禁用，以防止误操作。
        -   **视觉反馈**: 鼠标悬停在可点击翻页的区域时，光标显示为手型（pointer），提示用户可以点击。
...
2.  **文本编辑**:
    ...
    -   **重要**: 文本修改保存后，需要调用后端接口清除该绘本已生成的PDF文件（如果存在），因为内容已发生变化。
3.  **图片编辑 (重新生成)**:
    ...
    -   **流程**:
        ...
        6.  **重要**: 图片修改保存后，需要调用后端接口清除该绘本已生成的PDF文件（如果存在），因为内容已发生变化。

**e. PDF下载功能**

1.  **下载按钮**:
    -   在顶部工具栏右侧，音频播放控件和编辑按钮之间，显示"下载PDF"按钮。
    -   按钮样式与其他功能按钮保持一致（灰色背景，圆角设计）。
2.  **下载流程**:
    -   **检查PDF是否已生成**:
        1.  用户点击"下载PDF"按钮后，前端首先调用后端接口，检查该绘本是否已经生成过PDF文件。
        2.  后端接口返回PDF的生成状态（`has_pdf: true/false`）和PDF文件的URL（如果已生成）。
    -   **情况一：PDF已存在**:
        1.  如果后端返回 `has_pdf: true` 和有效的PDF URL，前端直接触发浏览器下载该PDF文件。
        2.  下载过程中，按钮文字变更为"下载中..."，按钮禁用，防止重复点击。
        3.  下载完成后，按钮恢复为"下载PDF"，并显示成功提示（Toast："PDF下载成功"）。
    -   **情况二：PDF未生成**:
        1.  如果后端返回 `has_pdf: false`，前端需要先请求后端生成PDF。
        2.  按钮文字变更为"生成中..."，按钮禁用。
        3.  调用后端PDF生成接口（`POST /api/storybook/{id}/generate-pdf`），后端开始异步生成PDF文件。
        4.  **轮询生成状态**: 前端每隔2-3秒轮询后端接口（`GET /api/storybook/{id}/pdf-status`），检查PDF生成进度。
        5.  **生成成功**: 当后端返回生成完成状态和PDF URL时，前端自动触发下载，按钮恢复，并显示提示（Toast："PDF生成完成，正在下载"）。
        6.  **生成失败**: 如果生成过程中出现错误，后端返回失败状态，前端显示错误提示（Toast："PDF生成失败，请稍后重试"），按钮恢复为"下载PDF"。
3.  **PDF内容要求**:
    -   PDF文件应包含绘本的所有页面，每页包含图片和对应的文字内容。
    -   PDF文件应保持原有的排版和设计风格。
    -   PDF文件名格式：`{绘本标题}_绘本.pdf`（例如："丑小鸭的春天_绘本.pdf"）。
4.  **缓存管理**:
    -   当用户在编辑模式下修改了绘本的图片或文字内容并保存后，前端需要调用后端接口清除该绘本已生成的PDF文件。
    -   清除接口：`DELETE /api/storybook/{id}/pdf`。
    -   这样可以确保下次下载时，用户获取到的是最新内容生成的PDF文件。

## 6. 附录：核心 AI 模型提示词

本章节详细记录了用于驱动AI绘本生成流程中各个关键环节的核心提示词（Prompts）。

### 6.1. 大纲生成模型 (`/generate-outline`)
...
此部分说明在脚本生成完成后，前端如何将结构化的脚本数据组合成最终用于图片生成的提示词（Prompt）。

**组合方式：**前端通过纯字符串拼接实现，无需调用AI模型。

#### 6.3.1. 最终提示词组合格式说明

在实际实现中，最终用于图片生成的提示词（Prompt）采用更简洁直观的格式，将脚本中的各个部分按固定顺序组合：
...
**具体步骤：**
1. **角色描述**：从 `character_models` 中找到该页面 `assets.characters` 列表中的所有角色，按顺序输出每个角色的 `{name}：{description}`
2. **场景描述**：从 `scene_models` 中找到该页面 `assets.scene` 对应的场景，输出 `场景：{description}`
3. **画面描述**：直接使用该页面 `directing.action_description` 的内容，输出 `画面：{action_description}`
4. **风格描述**：添加用户选择的绘画风格，输出 `采用{style_name}：{style_description}`
...
{
  "character_models": [
    {"name": "小牧童", "description": "一个约九岁的男孩，身材瘦小，皮肤健康的小麦色..."},
    {"name": "羊群", "description": "一群温顺的绵羊，毛发蓬松洁白..."}
  ],
  "scene_models": [
    {"name": "翠绿的山坡", "description": "一片宽广的山地，地势平缓..."}
  ],
  "pages": [
    {
      "page_number": 2,
      ...
      "directing": {
        "action_description": "小牧童坐在草地上，身体微微前倾，左手托着下巴..."
      }
    }
  ]
}
...
最终生成的提示词：
```
小牧童：一个约九岁的男孩，身材瘦小，皮肤健康的小麦色。他的脸庞圆润，眼睛大而明亮，眼神中透露着机灵。鼻头小巧，嘴唇薄。头发乌黑，短而微卷。他穿着一件蓝色粗布短袖上衣，下身是一条棕色短裤，脚上穿着草鞋。腰间系着一根麻绳，手持一根用于赶羊的细木棍。
羊群：一群温顺的绵羊，毛发蓬松洁白，体型圆润。它们的眼睛乌黑，耳朵竖直，鼻子粉嫩。四肢短小，尾巴短短地垂着。它们悠闲地吃着草，表情宁静。
场景：一片宽广的山地，地势平缓，覆盖着茂盛的翠绿色草地。草地上点缀着零星的黄色和紫色野花。远处是连绵起伏的青色山峦，近处有几棵高大的乔木，树叶翠绿。天空湛蓝，有几朵洁白的云彩。整体色彩鲜明，充满生机。
画面：小牧童坐在草地上，身体微微前倾，左手托着下巴，右手紧握木棍，眉头微皱，嘴巴紧闭，眼神中透露着一丝狡黠。几只羊在他身后不远处，继续低头吃草。
采用儿童插画风：明亮、色彩鲜艳、线条圆润、充满童趣的绘本风格。
```

**注意事项：**
- 此格式直接对应前端实现，便于代码拼接和调试
- 每个部分之间用换行符分隔，保持清晰的层级结构
- 角色描述按照 `assets.characters` 数组中的顺序输出
